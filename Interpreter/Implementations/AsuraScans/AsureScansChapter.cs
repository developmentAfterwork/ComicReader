...
namespace ComicReader.Interpreter.Implementations.AsuraScans
{
	...
	{
		public AsureScansChapter(string? id, string source, string mangaName, string title, string homeUrl, string lastUpdate, RequestHelper requestHelper, HtmlHelper htmlHelper) : base(id, title, homeUrl, lastUpdate, mangaName, source, requestHelper, htmlHelper) { }
		...
		public override async Task<List<string>> ImplGetPageUrls()
		{
			...
			var matches = Regex.Matches(part, "https[:\\/.a-z0-9\\-]*(.webp|.jpg)");
