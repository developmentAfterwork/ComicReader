...
namespace ComicReader.Interpreter.Implementations.MangaDex
{
	public class MangaDexReader : IReader
	{
		...
		public async Task<List<IManga>> LoadUpdatesAndNewMangs()
		{
			...
			try {
				var url = "https://api.mangadex.org/manga?limit=20&order%5BlatestUploadedChapter%5D=desc";
				//url = "https://api.mangadex.org/manga?limit=20";
				var result = await _requestHelper.DoGetRequest(url, 3, false, _timeout);
